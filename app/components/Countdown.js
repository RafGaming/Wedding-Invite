...
export default function Countdown() {
  ...
  const [weddingDateLabel, setWeddingDateLabel] = useState("December 31, 2026 (Placeholder)");
      ...
      .then((data) => {
        if (data.wedding_date) {
          ...
          if (!isNaN(parsed)) {
            ...
            setWeddingDateLabel(data.wedding_date);
          }
          if (data.wedding_time && data.wedding_date) {
            const combined = new Date(`${data.wedding_date}T${data.wedding_time}`);
            if (!isNaN(combined)) {
              setWeddingDate(combined);
            }
          }
        }
      })
  ...
  return (
    <section className="countdown-section scroll-reveal">
      <div className="countdown-inner">
        ...
        <p className="countdown-note accent-text">{weddingDateLabel}</p>
      </div>
    </section>
  );
}
